...
test('service worker refreshes audio online and falls back to the newest cached copy offline',async()=>{
  ...
  assert.equal(skipped,true);assert.equal(cacheStores.get('lets-party-v10').size,precacheUrls().length);
  ...
  cacheStores.set('lets-party-v9',new Map());
  listeners.activate({waitUntil(promise){pending=promise;}});await pending;
  assert.equal(claimed,true);assert.equal(cacheStores.has('lets-party-v9'),false);
  ...
  assert.equal(await (await cacheApi('lets-party-v10').match(replacedUrl)).text(),'new narration with unchanged filename');
